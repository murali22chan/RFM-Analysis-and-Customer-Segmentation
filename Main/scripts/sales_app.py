...
def load_data():
    return pd.read_csv('./sales_data1.csv')
...
if status == 'What were the best and worst months for sales?':
    ...
    p = create_figure(TOOLS=TOOLS,x_label='Months',y_label='Sales in INR (Rs)',height=400,x_ticks=months)
    ...
    h = hover([('Month', "@month"), ('Sales', "Rs@Sales")])
    p.add_tools(h)
...
#-------2. Which cities had the highest and lowest sales?--------------------
    ...
    p = create_figure(TOOLS=TOOLS,x_label='Cities',y_label='Sales in INR (Rs)',height=400,x_ticks=group_city['City'])
    p.vbar(x='City', top='Sales', width=0.9, source=source, fill_color=factor_cmap('City', palette=Paired12, factors=group_city['City']))

    h = hover([('City', "@City"),('Sales', "Rs@Sales")])
    p.add_tools(h)
